...
const Logo = ({ size, onClick }) => {
    return (
        <div onClick={onClick} className={`logo ${size}`}>
            <svg
                xmlns="http://www.w3.org/2000/svg"
                viewBox="0 0 64 64"
                fill="white"
            >
                <path fill="#FFAC46" d="M32 6C18.745 6 8 13.82 8 23h48c0-9.18-10.745-17-24-17z" />
                <path fill="#7B3F00" d="M4 27h56c2.21 0 4 1.79 4 4s-1.79 4-4 4H4c-2.21 0-4-1.79-4-4s1.79-4 4-4z" />
                <path fill="#FFDA9E" d="M8 39h48v2c0 5.523-10.745 10-24 10S8 46.523 8 41v-2z" />
                <circle cx="20" cy="18" r="2" fill="#fff" />
                <circle cx="28" cy="16" r="1.5" fill="#fff" />
                <circle cx="36" cy="18" r="2" fill="#fff" />
                <circle cx="44" cy="16" r="1.5" fill="#fff" />
            </svg>
        </div>
    );
};
...
const Logo = ({ size, onClick }) => {
  return (
    <img
      onClick={onClick}
      className={`logo ${size}`}
      src="/burguer.png" // Ruta relativa desde la carpeta 'public'
